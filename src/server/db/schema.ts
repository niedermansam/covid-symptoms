...
export const symptomsRelations = relations(symptoms, ({ many }) => ({
  symptomsToCases: many(cases),
}));

export const symptomsToCases = mysqlTable("symptomsToCases", {
  symptomId: varchar("symptomId", { length: 255 }).notNull(),
  caseId: varchar("caseId", { length: 255 }).notNull(),
}, (t) => ({
  pk: primaryKey(t.symptomId, t.caseId),
}));

export const symptomsToCaseRelations = relations(symptomsToCases, ({one}) => ({
  case: one(cases, {fields: [symptomsToCases.caseId], references: [cases.id]}),
  symptom: one(symptoms, {fields: [symptomsToCases.symptomId], references: [symptoms.id]}),
}))
